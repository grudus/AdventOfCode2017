...
object Day10 {
    private val array = (0..255).toList().toTypedArray()

    fun firstStar(input: List<Int>): Int =
            createHash(array.copyOf(), 1, input, 0, 0, input)
                    .let { it[0] * it[1] }
    ...
    fun secondStar(input: String, base: Array<Int> = array): String {
        ...
        return (0 until 16).fold("") { hash, hashIndex ->
            val char = (1 until 16).fold(hashed[16 * hashIndex]) { acc, i -> acc xor hashed[16 * hashIndex + i] }
            hash + char.toString(16).let { if (it.length == 1) "0$it" else it }
        }
    }
    ...
    tailrec fun createHash(data: Array<Int>, iterations: Int, lengths: List<Int>, index: Int, skip: Int, originalList: List<Int>): List<Int> {
        ...
        for (i in 0 until (len / 2)) {
            data.swap((i + index) % data.size, (index + len - 1 - i) % data.size)
        }

        return createHash(data, iterations, lengths.drop(1), (index + skip + len) % data.size, skip + 1, originalList)
    }
...
fun main(args: Array<String>) {
    ...
    println(Day10.firstStar(input.split(",").map { it.toInt() }))
    println(Day10.secondStar(input))
}