...
object Day15 {
    data class Generators(private val aValue: Long, private val bValue: Long) {
        fun next() = Generators((aValue * 16807) % 2147483647, (bValue * 48271) % 2147483647)
        fun matches() = toBinary(aValue, 32).substring(16) == toBinary(bValue, 32).substring(16)
    }

    fun firstStar(input: Generators) =
            (1..40_000_000).fold(Pair(input.next(), 0),
                    { (gen, num), _ -> Pair(gen.next(), if (gen.matches()) num + 1 else num) }).second
}
...
fun main(args: Array<String>) {
    val input = Day15.Generators(703, 516)
